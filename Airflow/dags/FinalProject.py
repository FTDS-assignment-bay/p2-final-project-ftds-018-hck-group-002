...
def load_csv_to_postgres():
    ...
    df = pd.read_csv('/opt/airflow/dags/FP_002_Bank_raw.csv')

    # Write the DataFrame to a table in the PostgreSQL database
    df.to_sql('table_fp', conn,  # "table_m3" is the name of the table
              index=False, 
...
def ambil_data():
    ...
    df = pd.read_sql_query("select * from table_fp", conn) 

    # Save the retrieved data to a CSV file at the specified location
    df.to_csv('/opt/airflow/dags/FP_002_Bank_new.csv',
...
def preprocessing(): 
    ...
    # Load the data
    data = pd.read_csv("/opt/airflow/dags/FP_002_Bank_new.csv")
    ...
    data.to_csv('/opt/airflow/dags/FP_002_Bank_clean.csv', index=False)
...
def upload_to_elasticsearch():
    ...
    df = pd.read_csv('/opt/airflow/dags/FP_002_Bank_clean.csv')
    
    # Loop through each row in the DataFrame
    for i, r in df.iterrows():
        doc = r.to_dict()  # Convert the row to a dictionary
        res = es.index(index="table_fp", id=i+1, body=doc)
...
default_args = {
    ...
    'start_date': datetime(2024, 8, 14, 12, 00) - timedelta(hours=7) # Start Date set in WIB
}

with DAG(
    "FP_002_DAG_HCK", # Project DAG Name
    description='Final_Project', # Project DAG description
    schedule_interval='30 6 * * *', # 6.30AM schedule interval
